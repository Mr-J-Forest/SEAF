...
\end{equation}
The SEAF-v1 LCFF control disables both the dedicated forcing representation and lead-conditioned aggregation, while retaining the raw external inputs, profile mixer, local--global blocks, spectral settings, four deterministic hypotheses, and the base spatial gate. This historical backbone contrast does not isolate either LCFF component and is not interpreted as a DynaSEAF component ablation.
...
The final forecast combines the direct prediction, transported prediction, and residual correction:
\begin{align}
R_h&=H_{\mathrm{innov}}(F,\widehat{D}_h,e_h),\nonumber\\
g_h&=\sigma\!\left(H_{\mathrm{gate}}(F,\widehat{D}_h,e_h)\right),\nonumber\\
\widehat{A}_h&=(1-g_h)\widehat{A}^{\mathrm{dir}}_h
       +g_h\widehat{A}^{\mathrm{trans}}_h+R_h.
\label{eq:dynaseaf_forecast}
\end{align}
The full configuration uses a target-resolved adaptive gate. The residual-correction and gate output layers are zero-initialized, and the gate prior is approximately $0.15$, which keeps the initial model close to the direct SEAF-v1 forecast while allowing lead- and location-dependent corrections.
...
\begin{figure*}[!t]
\centering
\subfloat[TEMP validation branches]{\includegraphics[width=0.46\textwidth]{dynaseaf_validation_temp_panel}}\hfill
\subfloat[SALT validation branches]{\includegraphics[width=0.46\textwidth]{dynaseaf_validation_salt_panel}}
