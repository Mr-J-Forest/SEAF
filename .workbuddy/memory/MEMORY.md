...
## 训练服务器
- 当前主力训练服务器（westb）：`connect.westb.seetacloud.com:42977`，root，RTX 4090 24GB，代码在 `/root/TSC-Fusion`（→ `/root/autodl-tmp/TSC-Fusion`）
- 同步：`export TSC_SERVER=root@connect.westb.seetacloud.com TSC_SERVER_PORT=42977 && ./sync_to_server.sh`（AGENTS.md 默认的 westd:39323 是旧服务器）
